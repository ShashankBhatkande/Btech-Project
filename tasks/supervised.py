...
class SupervisedForecastTask(pl.LightningModule):
    def __init__(
        ...
        regressor="linear",
        loss="mse",
        pre_len: int = 6,  # Changed from 3 to 6 for 30-minute prediction horizon
    ...
    def shared_step(self, batch, batch_idx):
        # x: (batch_size, seq_len, num_nodes, num_features), y: (batch_size, pre_len, num_nodes)
        x, y = batch
        num_nodes = x.size(2)
        predictions = self(x)  # (batch_size, num_nodes, pre_len)
        predictions = predictions.transpose(1, 2).reshape((-1, num_nodes))  # (batch_size * pre_len, num_nodes)
        y = y.reshape((-1, num_nodes))  # (batch_size * pre_len, num_nodes)
